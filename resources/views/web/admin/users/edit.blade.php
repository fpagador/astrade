...
@section('content')
    ...
    <x-form.form-wrapper action="{{ route('admin.users.update', $user->id) }}" method="POST" class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <!-- DNI -->
            <x-form.input label="DNI" name="dni" required value="{{ old('dni', $user->dni) }}" tooltip="Debe contener 8 números y 1 letra (ejemplo: 12345678A)." />
            <!-- Email -->
            <x-form.input label="Email" name="email" required value="{{ old('email', $user->email) }}" tooltip="Debe contener un @ y un . para ser válido." />
            <!-- Phone -->
            <x-form.input label="Teléfono" name="phone" required value="{{ old('phone', $user->phone) }}" type="number" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0,9)" tooltip="Debe contener 9 dígitos." />
            <!-- Username -->
        ...
        <div class="grid gap-4 md:grid-cols-2 mt-6 user-only">
            ...
            <x-form.input
                ...
                type="date" value="{{ old('contract_start_date', optional($user->contract_start_date)->format('Y-m-d')) }}"
            />
            <!-- Notification type -->
            <x-form.select
    ...
    <script>
        ...
        photoInput.addEventListener('change', () => {
            photoFilename.textContent = photoInput.files.length > 0 ? photoInput.files[0].name : 'Ningún archivo seleccionado';
        });
    </script>
